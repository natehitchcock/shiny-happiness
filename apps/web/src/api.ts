...
export interface CardDetail extends Card {
  printings: {
     ...
     *
     * Read by `artFromPrintings`, which is the fallback for a card the
     * hydration maps never covered — a "Cards named like…" result. Without it
     * the flip control would appear for a card in the deck and vanish for the
     * same card reached through search (ADR-0030).
     */
...
export interface ImageUris {
   ...
   *
   * Read by `viewBackImageUris` in `App.tsx`, which maps it onto
   * `CardView.backImageUris` for the flip control (ADR-0030).
   *
   * ABSENT means one physical face, which is most cards. PRESENT with nulls
   * means there IS a second side and its art has not resolved — the control
   * still draws, over an honest "no picture of this face" panel, rather than
   * hiding itself. `viewBackImageUris` is deliberately NOT `viewImageUris`
   * for that reason: the front collapses a pair of nulls to `undefined` and the
   * back must not, because absence of the key is the only spelling of "one
   * face".
   *

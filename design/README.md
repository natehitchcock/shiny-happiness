...
The `.dc.html` artboards are generated by `generate-artboards.py`, which holds the
shared design tokens, the card/meter/group components and the artboard layouts in
one place. Edit that rather than the artboards by hand — a token change touches
all fourteen otherwise.

```bash
python3 design/generate-artboards.py       # rewrites the .dc.html files
```

The seeded canvas file (`commander-deck-builder-ui.html`) is build output and is
gitignored — it is ~2 MB of embedded editor.

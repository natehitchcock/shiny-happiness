...
**⚠️ A CARDS RE-INGEST IS DUE for ADR-0059** — `synergy_produces` and
`synergy_wants` are stored columns, and `self-lifeloss` is a new value in an
existing `text[]`, so no migration. ADR-0057 §13 and ADR-0058 §8 were separately
due and have already been run; the note below survives because the `ramp` story
is worth keeping, not because it is outstanding. The `ramp` rule demanded

...
| Deck import from URL | Moxfield | **Not implemented.** Adapter seam exists, throws `NotAuthorizedError`, pending API access |
...
- EDHREC dependency is the fragile one: undocumented endpoints, no contract. Every
  ...
  not bolted on.
- Declining to scrape Moxfield costs the URL-import convenience and some
  bracket×colour correlation data. The text importer covers the actual user need,
  and EDHREC plus our own corpus covers the statistics. Worth it: building on
  access we do not have is not a foundation.
...
- **Scraping Moxfield anyway.** Rejected: they actively block it and have asked
  third parties to stop. Routing around access controls is not something this
  project does, and it would be a fragile foundation even if it were.
